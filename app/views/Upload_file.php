...
if(move_uploaded_file($_FILES['uploadedfile1']['tmp_name'], $target_path1)) {
    ...
    $result = mysql_connect("localhost", "root", "Xerxes641602") or die ("Could not save image name Error: " .
        mysql_error());
    mysql_select_db("cdcol") or die("Could not select database");
    mysql_query("INSERT into images (path) VALUES('".$_FILES['uploadedfile1']['name']."')");
...
if(move_uploaded_file($_FILES['uploadedfile2']['tmp_name'], $target_path2)) {
      $result = mysql_connect("localhost", "root", "Xerxes641602") or die ("Could not save image name Error: " .
        mysql_error());
    mysql_select_db("cdcol") or die("Could not select database");
    mysql_query("INSERT into images (URL) VALUES('".$_FILES['uploadedfile2']['name']."')");
